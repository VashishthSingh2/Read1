...
<%@taglib uri="http://www.springframework.org/tags/form" prefix="form" %>
 <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form:form method="post" id="deleteBinForm" name="deleteBinForm" action="${pageContext.request.contextPath}/admin/deleteBin">
        <form:input type="hidden" path="id" id="id" class="form-control" value="${bin.id}"/>
          <button class="btn btn-danger" type="button" data-dismiss="modal" onclick="$('#deleteModal').html('').css({display:'none'});$('.modal-backdrop').remove();$('#deleteModal').addClass('bg-blur');">No</button>
           <button class="btn btn-success" type="button" onclick="deleteBin()">Yes</button>
          </form:form>
         </div>
      </div>
    </div>